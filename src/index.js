      ...
      function generateUserName(){ 
        ...
        var makeUsername = '';
         ...
         for(let i=0; i<4;i++){ 
             ...
             makeUsername += alphabet[index]; } for(let j=0;j<4;j++){ makeUsername += "*"; }
  ...
  //event.target.parentNode.id 값이 string이기 때문에 이 값을 Number처리하던지, idOrVoteCountList[i]["commentId"]이 값을 string처리해야 넘어감.
        ...
        if(idOrVoteCountList[i]["commentId"].toString() === event.target.parentNode.id){
           idOrVoteCountList[i]["voteUpCount"]++; event.target.innerHTML = "👍"+idOrVoteCountList[i]["voteUpCount"]; } 
            ...
            function deleteComments(){ const btn = event.target; const list = btn.parentNode.parentNode.parentNode;
                        //commentList 
                        //삭제버튼도 마찬가지임. 여러개니깐 인식을 못함. 상위노드에 id 부여함. 
                        //삭제버튼 클릭한 배열의 인덱스를 날리면 됨. 뭐 여기까지 해도 상관없는데... 
                        
                for(let i=0; i<idOrVoteCountList.length; i++){ if(idOrVoteCountList[i]["commentId"].toString() === btn.parentNode.id){ idOrVoteCountList.splice(btn.parentNode.id-1,1); } }
                //그다음에 전체 지우기.
                rootDiv.removeChild(list);
                //메인댓글 카운트 줄이기.
                if(mainCommentCount.innerHTML <='0'){ mainCommentCount.innerHTML = 0;
           ...
           const newId = idOrVoteCountList.length+1;
              ...
              spacer.className = "spacer";
                    ...
                    function pressBtn(){ 
                        ...
                        const forbidden = new Array('mother','father');
                        let tap;
                        
                        for(i=0; i<forbidden.length; i++){
                            tap = inputBar.value.toLowerCase().indexOf(forbidden[i]);
            ...
            console.dir(mBtn.parentNode.parentNode.parentNode.children[1]);

            for(var i=0; i<idOrVoteCountList.length; i++){
            if(idOrVoteCountList[i]["commentId"].toString() === mBtn.parentNode.parentNode.id){
                 idOrVoteCountList[i]["commentValue"] = input.value;

    //수정버튼누르면 변경.
    mBtn.parentNode.parentNode.parentNode.children[1].innerHTML = input.value;
    modal.classList.add("hidden"); } } }); }
